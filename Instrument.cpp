...
void initialization(int **edge, int n)
{
	...
	while (!inputfile.eof())
	{
		...
		if (!(judgeflag (s1)) && (s2.compare(0,3,": L") == 0)) edge[i][i+1] = 1;
		else
		{
...
void ModifyIR(int **edge, int n)
{
	...
	while (!inputfile.eof())
	{
		...
		if (!(judgeflag (s1)) && (s2.compare(0,3,": L") == 0))
		{
			...
			if (judgeflag (s1) && (s1.compare(0,3,": L") != 0))
			  ...
			  else
			  {
				  ...
				  if (!judgeflag(s2))
				  {
					  ...
					  outputfile4 << "\t+\ttt" << (i-1)*n+j << ", _c" << (i-1)*n+j << ", 1" << endl;
					  outputfile4 << "\t=\t_c" << (i-1)*n+j << ", tt" << (i-1)*n+j << endl;
					  outputfile4 << "\t:=\t" << s1.substr(s1.find("L"), s1.find(",")-s1.find("L")) << endl;
